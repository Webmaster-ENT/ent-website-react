...
export default function TabsRegistration() {
  ...
  const performCheck = async (nrpValue: string) => {
    if (!nrpValue) return;
    const result = await checkNRP(nrpValue);
    switch (result) {
      case "checking":
        break;
      case "registered":
  ...
  const handleRegistrationCheck = async (e: FormEvent) => {
    e.preventDefault();
    await performCheck(nrp);
  };

  // Auto check when search query param has NRP
  useEffect(() => {
    const nrpParam = searchParams.get("nrp");
    if (nrpParam) {
      setNrp(nrpParam);
      performCheck(nrpParam);
    }
  }, [searchParams]);
